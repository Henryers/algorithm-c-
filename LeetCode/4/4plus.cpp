...
public:
    ...
    int getKthElement(const vector<int>& nums1, const vector<int>& nums2, int k) {
         ...
         * 如果 pivot = mid1，那么 nums1[0 .. k/2-1] 都不可能是第 k 小的元素。把这些元素全部 "删除"，剩下的作为新的 nums1 数组
         * 如果 pivot = mid2，那么 nums2[0 .. k/2-1] 都不可能是第 k 小的元素。把这些元素全部 "删除"，剩下的作为新的 nums2 数组
        ...
        while (true) {
            ...
            if (mid1 <= mid2) {
                // 淘汰了几个？ [已淘汰]【index1  ... newIndex1】[newIndex1 + 1, 后面部分]，中间【】就是淘汰部分！
                k -= newIndex1 - index1 + 1;
                index1 = newIndex1 + 1;  // 新的起点
            }
            else {
